...
export async function createNewDoctor(data: any) {
  try {
    ...
    console.log("Doctor created in DB:", doctor.id);
      ...
      await Promise.all(
        workingDayData.map((el) =>
          db.workingDays.create({
            ...
            console.log("Working day created:", el.day);
            return result;
          })
        )
      );
    } catch (workingDaysError) {
      console.error("Error creating working days:", workingDaysError);
      // Optionally delete the doctor/user if partial failure is unacceptable
      return { error: true, success: false, message: "Failed to set working days" };
    }
    ...
    if (error.errors?.[0]?.code === 'form_identifier_exists') {
      return { error: true, success: false, message: "Email address is already taken" };
    }
    return { error: true, success: false, message: "Something went wrong" };
  }
}
