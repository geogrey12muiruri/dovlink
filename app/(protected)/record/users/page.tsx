...
const UserPage = async () => {
  ...
  if (!client || !client.users) {
    console.error("Clerk client initialization failed:", client);
    return <div>Error: Unable to fetch users. Please try again later.</div>;
  }
  ...
  try {
    ...
    const renderRow = (item: UserProps) => (
      ...
      >
        ...
        <td className="hidden md:table-cell capitalize">
          {item?.lastSignInAt
            ? format(new Date(item.lastSignInAt), "yyyy-MM-dd h:mm:ss")
            : "Never"}
        </td>
      </tr>
    );
    ...
    return <div>Error: Failed to load users. Please try again.</div>;
  }
};
